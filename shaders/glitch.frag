...
void main() {
  ...
  float time = cubicPulse(0.5, .05, fract(uTime / 4.0));
  float glitchScale = mix(0.0, 8.0, time);
  float aberrationSize = mix(0.0, 2.0, time);
  ...
  vec2 greenOffset = vec2(-aberrationSize, 0.0) / uResolution;
